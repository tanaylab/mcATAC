...
available_datasets
sample_inds <- sample(nrow())
motif_mat = generate_motif_pssm_matrix(peak_set = mc_atac@peaks, datasets_of_interest = "jaspar", parallel = T)
head(motif_mat)
```
...
```{r random_genome}
gset_genome("hg38")
rg = gen_random_genome_peak_motif_matrix(num_peaks = 1e+3,
...
ks_all = calculate_d_stats(motif_mat, pssm_bg, fg_clustering = NULL, parallel = TRUE, alternative = "less", nc = parallel::detectCores()) {
head(rg)
```