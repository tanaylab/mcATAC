...
```{r plot-peak-density}
plot_peak_coverage_density(atac_sc)
```
...
```{r filter-peaks}
atac_sc <- filter_features(scatac = atac_sc, minimal_max_umi = 3, min_peak_length = 200, max_peak_length = 1000, max_peak_density = 300)
```
...
plot_atac_rna(atac_mc, "CD4", "CD8A")
```
