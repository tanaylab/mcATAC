...
#' @param atac_sc the ScPeaks object to analyze
...
#'
#' @return atac_sc - the filtered ScPeaks object
#' @examples
#' \dontrun{
...
#' @param atac_mc the McPeaks object to analyze
...
#' @param atac (optional) an ScPeaks or McPeaks object
...
find_blacklist_overlaps <- function(atac = NULL, peaks = NULL, genome = NULL, max_dist_to_blacklist_region = 0, blacklist_name = "ENCODE.blacklist") {
    ...
    if (!is.null(atac) && is.null(peaks)) {
        ...
        cli_abort("Must specify either ScPeaks, McPeaks or peaks")
