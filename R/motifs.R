...
#'
#' @param atac - an ScPeaks/McPeaks or PeakIntervals object
...
#' @param pssm_fg motif energies calculated for a certain set of motifs on a PeakIntervals/ScPeaks/McPeaks object
...
get_peaks_for_pssm <- function(atac) {
    cl <- class(atac)
    if (any(c("ScPeaks", "McPeaks") %in% cl[[1]])) {
        peaks <- atac@peaks
    } else if ("PeakIntervals" %in% cl[[1]]) {
        peaks <- atac
    } else {
        cli_abort("Class of {.var atac} is not recognized (should be either ScPeaks, McPeaks or PeakIntervals object).")
    }
    return(peaks)
}
