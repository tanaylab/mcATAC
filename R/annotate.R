...
annotate_peaks <- function(atac) {
    if (!methods::is(atac, "ATACPeaks")) {
        cli_abort("{.field atac} is not an ScATACPeaks or McATACPeaks object. You can annotate intervals directly using the {.code annotate_intervals} function.")
    }
...
annotate_intervals <- function(intervals, genome,
    ...
    if (missing(genome)) {
        cli_abort("Please Specify genome. Look for slot '@genome' in relevant McATACPeaks/ScATACPeaks object.")
    }
...
#'
#' @param atac a McATACPeaks or ScATACPeaks a PeakIntervals object or a misha intervals set
...
name_enhancers <- function(atac, tad_names = gintervals.load("intervs.global.tad_names")) {
    cl <- class(atac)
    if (cl[[1]] %in% c("ScATACPeaks", "McATACPeaks")) {
        peaks <- atac@peaks
        ...
        cli_abort("Class of {.var atac} is not recognized (should be either ScATACPeaks, McATACPeaks, PeakIntervals object or misha intervals).")
    }
