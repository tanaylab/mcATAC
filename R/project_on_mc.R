#' Given metacells (usually from RNA data), project ATAC counts to get a McATACPeaks object
#'
#' @description Given cell to metacell association, summarise atac counts to generate a McATACPeaks object.
...
#' @param min_int_frac (optional) minimal expected fraction of intersection of barcodes (cell names) in ScATACPeaks
...
project_atac_on_mc <- function(atac, cell_to_metacell = NULL, metadata = NULL, min_int_frac = 0.5, mc_size_eps_q = 0.1, id = NULL, description = NULL, rm_zero_peaks = TRUE, ignore_metacells = -1) {
    ...
    id <- id %||% atac@id
    mc_atac <- new("McATACPeaks", mat = mc_mat, peaks = atac@peaks[non_zero_peaks, ], genome = atac@genome, metadata = metadata, cell_to_metacell = enframe(cell_to_metacell, "cell_id", "metacell"), mc_size_eps_q = mc_size_eps_q, id = id, description = description, tad_based = atac@tad_based)
