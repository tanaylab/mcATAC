...
normalize_marginal <- function(orig_track, norm_track, window_size = 20e3, iterator = 1e3, floor_percentile = 0.1, overwrite = FALSE) {
    ...
    floor_val <- gquantiles(smoo_track, floor_percentile)
    ...
    expr <- glue("{orig_track} / 2^({smoo_track_floored} - 1)", smoo_track_floored = glue("pmax(1, 1 + ifelse(is.na({smoo_track}), 0, {smoo_track} - {floor_val}))"))
